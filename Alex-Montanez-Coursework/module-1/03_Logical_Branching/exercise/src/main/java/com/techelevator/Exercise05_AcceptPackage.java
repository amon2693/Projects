...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches) {
        int boxDimenson = lengthInches * widthInches * heightInches;
        if ((weightPounds <= 40) && (boxDimenson <= MAX_CUBIC_INCHES)) {
            return true;
        }
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        ...
        int boxDimenson = lengthInches * widthInches * heightInches;
        int packageHeight = MAX_DIMENSION_INCHES;
        if ((weightPounds <= 40) && (boxDimenson <= MAX_CUBIC_INCHES) && (packageHeight <= 66 && isSurchargePaid)) {
            return true;
        }
        return false;
