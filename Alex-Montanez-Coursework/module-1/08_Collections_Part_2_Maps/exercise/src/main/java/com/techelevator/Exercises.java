...
public class Exercises {
	...
	public double isItOnSale(Map<String, Double> itemsOnSale, String itemNumber) {
		...
		String itemNumberUpperCase = itemNumber.toUpperCase();

		if (itemsOnSale.containsKey(itemNumberUpperCase)) {
			return itemsOnSale.get(itemNumberUpperCase);
		} else {
			return 0.00;
		}
	}
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		if (peterPaul.containsKey("Peter") && peterPaul.containsKey("Paul")) {
			...
			if (peterMoney > 0 && paulMoney < 1000) {
				int transferAmount = Math.min(peterMoney / 2, paulMoney + peterMoney);
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if (peterPaul.containsKey("Peter") && peterPaul.containsKey("Paul")) {
			...
			if (peterMoney >= 5000 && paulMoney >= 10000) {
				...
				peterPaul.put("Partnership", totalContribution);
			}
		}
		return peterPaul;
	}
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		Map<Integer, Integer> newIntegerCount = new HashMap<>();
		int i = 0;
		for (int num : ints) {
			if (newIntegerCount.containsKey(num)) {
				newIntegerCount.put(num, newIntegerCount.get(num) +1);

			} else {
				newIntegerCount.put(num, 1);
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		Map<String, Boolean> isMoreThanOnce = new HashMap<>();
		for (String word: wordCount.keySet()) {
			boolean moreThanOnce = wordCount.get(word) >= 2;
			isMoreThanOnce.put(word, moreThanOnce);
		}
