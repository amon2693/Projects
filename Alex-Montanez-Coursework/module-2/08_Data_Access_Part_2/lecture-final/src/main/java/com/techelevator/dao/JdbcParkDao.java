...
public class JdbcParkDao implements ParkDao {
    ...
    public void linkParkState(int parkId, String stateAbbreviation) {
        String sqlExists = "SELECT COUNT(*) AS count FROM park_state WHERE park_id = ? AND state_abbreviation = ?;";
        String sqlLink = "INSERT INTO park_state (park_id, state_abbreviation) " +
                "VALUES (?, ?)";
        try {
            int count = jdbcTemplate.queryForObject(sqlExists, int.class, parkId, stateAbbreviation);
            if (count < 1) {
                jdbcTemplate.update(sqlLink, parkId, stateAbbreviation);
            }
        }
        catch (CannotGetJdbcConnectionException e) {
            throw new DaoException(DaoException.NO_DATABASE_ERROR, e);
