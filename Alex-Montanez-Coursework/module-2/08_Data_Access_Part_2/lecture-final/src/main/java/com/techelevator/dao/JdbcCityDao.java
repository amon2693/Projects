...
public class JdbcCityDao implements CityDao {
    ...
    public City someoneMovedToCity(int cityId) {
        City updatedCity = null;
        String sql = "UPDATE city SET population = population + 1 " +
                "WHERE city_id = ?;";
        try {
            int numberOfRows = jdbcTemplate.update(sql, cityId);

            if (numberOfRows == 0) {
                throw new DaoException("Zero rows affected, expected at least one");
            } else {
                updatedCity = getCityById(cityId);
            }
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Unable to connect to server or database", e);
        } catch (DataIntegrityViolationException e) {
    ...
    public String getCatNoise() {
        if (recentCityId == 2) {
            throw new DaoException("Cats don't make noise in Akron");
        }
        return "Meow!";
    }
