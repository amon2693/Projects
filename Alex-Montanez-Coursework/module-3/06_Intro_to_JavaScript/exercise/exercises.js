...
function frontAgain(word) {
	//return word.substring(0,2) === word.endsWith(word.substring(word.length -2));
	if(word.length == 2) {
		return true;
	}
	const front = word.substring(0,2);
	const end = word.substring(word.length -2);
...
function cigarParty(cigars, isWeekend) {
    ...
    if (isWeekend) {
        ...
        return cigars >= 40 && cigars <= 60; // Must be between 40 and 60 on weekdays
    }
}
...
*/
	...
	return evens.filter(number => number % 2 === 0);
	}
...
function filterBigNumbers(big) {
	return big.filter(number => number >= 100);
}
...
function filterMultiplesOfX(arr, x) {
    // Use the filter method to return only multiples of x
